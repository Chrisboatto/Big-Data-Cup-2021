...
```{r}
ottersData$X.Coordinate <- as.numeric(ottersData$X.Coordinate) - 100
ottersData$Y.Coordinate <- as.numeric(ottersData$Y.Coordinate) - 42.5
```

```{r}
ottersOffense <- filter(ottersData, Event == "Goal" | Event == "Shot" | Event == "Dumped In" | Event == "Carried In")
```


```{r}
ggplot(ottersOffense, aes(x = X.Coordinate, y = Y.Coordinate)) + 
  gg_rink(side = "left") +
  gg_rink(side = "right") +
  geom_point(aes(color = Event, shape = Event), position = "jitter", size = 1.0, alpha = 0.7) + 
  labs(title = "Otters Chart: Offense for Dumped In vs Carry", x = NULL, y = NULL) + 
  scale_colour_manual(values = c("Goal" = "red", "Shot" = "blue", "Carried In" = "green" , "Dumped In" = "yellow"), name = NULL) +
    scale_shape_manual(values = c("Goal" = 4, "Shot" = 16, "Carried In" = 12, "Dumped In" = 10), name = NULL) +
...
```{r}
attach(ottersData)
fullModel <- randomForest(Goal ~ X.Coordinate + Y.Coordinate  + Takeaway + Puck_Recover + Period + Penalty + Pass_Inc + Pass_Comp + game_date + Faceoff_Win + Dumped.In + Clock + Carried.In, ntree = 100, maxnodes = 20)
```
